...
def plot_from_pose(seq, dir_save, pose_abs, epoch=None, args=None):
    ...
    plt.close('all')
    style.use("ggplot")
    pose_gt = np.loadtxt('D:/sai/label/ground_truth/{:02d}.txt'.format(seq))
...
def plot_from_pose_(seq, dir_save, pose_abs):
    """
    训练和测试时画图的legend、命名、保存位置不一样
    """
    plt.close('all')
    style.use("ggplot")
    pose_gt = np.loadtxt('D:/sai/label/malaga_orb_pose/{:02d}.txt'.format(seq))
    pose_pre = np.array(pose_abs)  # [image_numbers, 6]
    plt.plot(pose_gt[:, 3], pose_gt[:, 11], '--', c='k', lw=1.5, label='Ground truth')

    plt.plot(pose_pre[:, 3], pose_pre[:, 11], '-', c='r', lw=1.5, label='estimated')
...
def main():
    ...
    for seq in range(11):
        ...
        style.use("ggplot")
        pose_gt = np.loadtxt('D:/sai/label/ground_truth/{:02d}.txt'.format(seq))
        with open('D:/sai/DeepVO-10.17/result/10.22/out_{:02d}.txt'.format(seq)) as f:
            pose_pre = np.array([[float(x) for x in line.split(',')] for line in f])
        plt.plot(pose_gt[:, 3], pose_gt[:, 11], '--', c='k', lw=1.5, label='Ground truth')
        plt.plot(pose_pre[:, 3], pose_pre[:, 5], '-', c='r', lw=1.5, label='RNN')
        ...
        plt.grid(True)
        plt.legend()
        # dir_save = 'D:/sai/DeepVO-10.17/result/10.22'
        # plt.savefig(dir_save + '/{:d}.png'.format(seq))
        # print(pose_pre)
        poses = np.zeros(pose_pre.shape)
        poses[:,0:3] = np.array(pose_pre[:,3:])
        poses[:, 3:] = pose_pre[:,0:3]
        # print(poses)
        np.savetxt('D:/sai/DeepVO-10.17/result/10.22/{:02d}.txt'.format(seq), poses)
