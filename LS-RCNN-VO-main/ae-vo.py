...
parser.add_argument('--resume', default='Yes', help='[ae] for cnn, [Yes / No] for cnn+fc')
# 权重恢复的路径
parser.add_argument('--net_restore', default='ae-vo', help='[cnn-vo /ae/ ae-vo/ rcnn-vo]')
parser.add_argument('--dir_restore', default='20201010-1', help='Restore time')
parser.add_argument('--model_restore', default='model-validate', help='Restore model-id')
# 保存权重的路径
parser.add_argument('--net_name', default='ae-vo', help='[cnn-vo /ae-vo/ rcnn-vo]')
parser.add_argument('--net_time', default='20201010-1', help='save time, such as 20200102')
# 训练参数
...
def main():
    ...
    elif args.resume == 'Yes' or args.phase == 'Test':
    ...
    if args.phase == 'Train':
        ...
        if args.resume == 'ae':
            lstm_fc_params = list(map(id, model.module.fc_1.parameters()))
            lstm_fc_params += list(map(id, model.module.fc_2.parameters()))
            lstm_fc_params += list(map(id, model.module.fc_3.parameters()))
            lstm_fc_params += list(map(id, model.module.fc_4.parameters()))
