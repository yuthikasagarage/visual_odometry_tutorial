...
from utils.post_process import cal_absolute_from_relative, plot_from_pose_

# 从deepvo工程中读取图片，然后在这里计算光流，通过恢复权重，预测位姿
...
if __name__ == '__main__':
	arguments_strModel = 'D:/sai/pytorch-ofvo/test/network-default.pytorch'
	cnn_restore = 'D:/sai/pytorch-ofvo/model/of-vo/20191204/model-60.pkl'
	dir_time = 'D:/sai/pytorch-ofvo/test/test'
	model = Net()
	if torch.cuda.is_available():
		model = nn.DataParallel(model.cuda())
	print('\n========================================')
	print('Restore from CNN: {:s}'.format(cnn_restore))
	model.load_state_dict(torch.load(cnn_restore))
	print('========================================')
	model.eval()

	for seq in range(22):
		img_list = glob( 'D:/sai/data_odometry_color/dataset/sequences/{:02d}/image_2/*.png'.format(seq))
		img_list.sort()
		pose_ret = []
		for j in range(len(img_list)-1):
			...
			tensorFirst = torch.FloatTensor(numpy.array(PIL.Image.open(arguments_strFirst).resize((1280, 384)))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
			tensorSecond = torch.FloatTensor(numpy.array(PIL.Image.open(arguments_strSecond).resize((1280, 384)))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
			# print(tensorFirst.shape)# torch.Size([3, 436, 1024])
			starttime = datetime.datetime.now()
			tensorOutput = estimate(tensorFirst, tensorSecond)
			# print(tensorOutput.shape)# torch.Size([2, 436, 1024])
			tensorOutput = tensorOutput.reshape(-1,2,384,1280)
			pose_pre =model(tensorOutput)
			endtime = datetime.datetime.now()
			pose_pre = pose_pre.data
			pose_ret.extend(pose_pre.cpu().numpy())
			print((endtime - starttime))
			# print(pose_pre)
		pose_abs = cal_absolute_from_relative(pose_ret)
		# numpy.savetxt(dir_time + '/pose_{:d}.txt'.format(seq), pose_ret)
		# numpy.savetxt((dir_time + '/{:02d}.txt'.format(seq)), pose_abs)
		plot_from_pose_(seq=seq, dir_save=dir_time, pose_abs=pose_abs)
