...
class Net(nn.Module):
    def __init__(self, batch_norm=False):
        ...
        self.lstm = nn.LSTM(input_size=1024*3*10, hidden_size=1000, num_layers=2, batch_first=True, dropout=0.5)
        self.dropout2 = nn.Dropout(0.5)
        self.fc_lstm_1 = fc(1000, 128, activation=True)
    ...
    def forward(self, x):
        ...
        x = x.reshape(N, -1, 1024*3*10)        # N x T x 30720
        x = self.dropout1(x)
        self.lstm.flatten_parameters()      # cuda加速
        x, _ = self.lstm(x)                 # N x T x 1000
        x = self.dropout2(x)
        x = x.contiguous().view(-1, 1000)   # (NxT)x1000
        x = self.fc_lstm_1(x)
        x = self.fc_lstm_2(x)
